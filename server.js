...
app.post('/api/crear-ofrecimiento', async (req, res) => {
  try {
    ...
    await db.collection('ofrecimientos-team-a-junio-2024').add(ofrecimiento);
...
app.get('/api/obtener-ofrecimientos', async (req, res) => {
  try {
    const ofrecimientosSnapshot = await db.collection('ofrecimientos-team-a-junio-2024').get();
...
app.put('/api/actualizar-ofrecimiento/:id', async (req, res) => {
  try {
    ...
    const ofrecimientoRef = db.collection('ofrecimientos-team-a-junio-2024').where('id', '==', ofrecimientoId);
