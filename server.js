...
app.post("/api/crear-ofrecimiento", async (req, res) => {
  try {
    ...
    await db.collection("ofrecimientos-team-a-julio-II-2024").add(ofrecimiento);
    res.json({ message: "Ofrecimiento creado exitosamente." });
  } catch (error) {
    console.error(error);
    res
      .status(500)
      .json({ error: "Ocurrió un error al crear el ofrecimiento." });
  }
});
...
app.get("/api/obtener-ofrecimientos", async (req, res) => {
  try {
    const ofrecimientosSnapshot = await db
      .collection("ofrecimientos-team-a-julio-II-2024")
      .get();
    ...
    res
      .status(500)
      .json({ error: "Ocurrió un error al obtener los ofrecimientos." });
  }
});
